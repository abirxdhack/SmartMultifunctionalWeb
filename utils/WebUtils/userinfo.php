...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    ...
    $api_url = "https://a360api-c8fbf2fa3cda.herokuapp.com/user/info?username=" . urlencode($username);
    $ch = curl_init();
    ...
    if ($data && isset($data['success']) && $data['success'] === true) {
        echo json_encode([
            ...
            'user' => [
                ...
                'dc_id' => isset($data['dc_id']) ? htmlspecialchars($data['dc_id']) : '',
                'dc_location' => isset($data['dc_location']) ? htmlspecialchars($data['dc_location']) : '',
                'is_premium' => isset($data['is_premium']) ? $data['is_premium'] : false,
                'is_verified' => isset($data['is_verified']) ? $data['is_verified'] : false,
                'is_bot' => isset($data['is_bot']) ? $data['is_bot'] : false,
                'flags' => isset($data['flags']) ? htmlspecialchars($data['flags']) : '',
                'status' => isset($data['status']) ? htmlspecialchars($data['status']) : '',
                'account_created' => isset($data['account_created']) ? htmlspecialchars($data['account_created']) : '',
                'account_age' => isset($data['account_age']) ? htmlspecialchars($data['account_age']) : '',
                'links' => isset($data['links']) ? array_map('htmlspecialchars', $data['links']) : []
            ],
...
<head>
  ...
  <meta name="description" content="Smart Telegram User Info: Retrieve Telegram user information with ease. Developed by Abir Arafat Chawdhury (@ISmartCoder).">
  <meta name="keywords" content="Telegram User Info, Telegram Lookup, @ISmartCoder">
  <meta name="author" content="Abir Arafat Chawdhury (@ISmartCoder)">
  <title>Smart Telegram User Info</title>
  ...
  <style>
    ...
    .download-button {
      background: linear-gradient(90deg, #00f7f9, #ee1d52);
      color: #ffffff;
    ...
    .download-button:hover {
      box-shadow: 0 0 15px rgba(0, 247, 249, 0.6);
...
<body>
  ...
  <header class="text-center py-8 sm:py-16">
    <img src="https://i.ibb.co/yBNCxwvM/enhanced.png" alt="Smart Telegram User Info Logo" class="logo">
    <div class="flex items-center justify-center mt-6 sm:mt-8">
      <h1 class="text-4xl sm:text-5xl md:text-6xl font-bold glow stylish-text mr-4">Smart Telegram User Info</h1>
    ...
    <p class="welcome-message mt-3 sm:mt-4 tracking-wide stylish-text text-lg sm:text-xl leading-relaxed max-w-5xl mx-auto">
      The Ultimate Telegram User Info Tool: Retrieve Telegram user information with ease. Built with passion by @ISmartCoder. Still under active development, daily updated by @TheSmartDev.
    </p>
  ...
  <section id="telegram-userinfo" class="section-card max-w-5xl mx-auto">
    <h2 class="text-3xl sm:text-4xl font-semibold text-center mb-8 sm:mb-10 glow stylish-text">Telegram User Info</h2>
    <div class="tool-card">
      <div class="tool-card-content">
        <div class="flex justify-center mb-8">
          <i class="fab fa-telegram text-6xl text-gray-600 animate-pulse"></i>
        </div>
        <h3 class="text-2xl font-bold stylish-text mb-8 text-center">Lookup Telegram User</h3>
        <div id="form-container" class="mb-8">
          <div class="grid grid-cols-1 max-w-md mx-auto">
            <div>
              <label class="block text-sm font-medium stylish-text mb-3">Telegram Username</label>
              <input type="text" id="username-input" class="input-field" placeholder="e.g., @ISmartCoder" required>
            </div>
          </div>
        </div>
        <div class="text-center">
          <button onclick="fetchUserInfo()" class="button font-bold">Get User Info</button>
        </div>
        <p id="error" class="error hidden"></p>
        <div id="results" class="hidden">
          <h4 class="text-2xl font-bold stylish-text mb-6 text-center">User Details</h4>
          <div class="result-box">
            <p><i class="fas fa-check-circle text-green-500"></i> <strong>Status:</strong> <span id="status"></span></p>
            <p><i class="fas fa-user text-blue-500"></i> <strong>ID:</strong> <span id="user-id"></span></p>
            <p><i class="fas fa-user text-blue-500"></i> <strong>First Name:</strong> <span id="first-name"></span></p>
            <p><i class="fas fa-user text-blue-500"></i> <strong>Last Name:</strong> <span id="last-name"></span></p>
            <p><i class="fas fa-at text-purple-500"></i> <strong>Username:</strong> <span id="username"></span></p>
            <p><i class="fas fa-server text-gray-500"></i> <strong>Data Center ID:</strong> <span id="dc-id"></span></p>
            <p><i class="fas fa-map-marker-alt text-red-500"></i> <strong>Data Center Location:</strong> <span id="dc-location"></span></p>
            <p><i class="fas fa-crown text-yellow-500"></i> <strong>Premium:</strong> <span id="is-premium"></span></p>
            <p><i class="fas fa-check text-green-500"></i> <strong>Verified:</strong> <span id="is-verified"></span></p>
            <p><i class="fas fa-robot text-gray-500"></i> <strong>Bot:</strong> <span id="is-bot"></span></p>
            <p><i class="fas fa-flag text-blue-500"></i> <strong>Flags:</strong> <span id="flags"></span></p>
            <p><i class="fas fa-clock text-purple-500"></i> <strong>Status:</strong> <span id="user-status"></span></p>
            <p><i class="fas fa-calendar text-blue-500"></i> <strong>Account Created:</strong> <span id="account-created"></span></p>
            <p><i class="fas fa-hourglass text-gray-500"></i> <strong>Account Age:</strong> <span id="account-age"></span></p>
            <p><i class="fas fa-link text-green-500"></i> <strong>Links:</strong> <span id="links"></span></p>
          </div>
        </div>
      </div>
    </div>
  ...
  <footer class="bg-gradient-to-r from-indigo-50 to-purple-50 py-10 sm:py-16 mt-auto">
    <div class="footer-container max-w-5xl mx-auto text-center">
      <div class="footer-content">
        ...
        <p class="text-base sm:text-lg font-bold stylish-text">© 2025 Smart Telegram User Info. All rights reserved.</p>
      </div>
    </div>
  </footer>
  <script>
    ...
    async function fetchUserInfo() {
      ...
      if (!username) {
        errorEl.textContent = 'Please enter a valid Telegram username.';
      ...
      try {
        ...
        if (data.error) {
          ...
          document.getElementById('dc-id').textContent = data.user.dc_id;
          document.getElementById('dc-location').textContent = data.user.dc_location;
          document.getElementById('is-premium').textContent = data.user.is_premium ? 'Yes' : 'No';
          document.getElementById('is-verified').textContent = data.user.is_verified ? 'Yes' : 'No';
          document.getElementById('is-bot').textContent = data.user.is_bot ? 'Yes' : 'No';
          document.getElementById('flags').textContent = data.user.flags;
          document.getElementById('user-status').textContent = data.user.status;
          document.getElementById('account-created').textContent = data.user.account_created;
          document.getElementById('account-age').textContent = data.user.account_age;
          document.getElementById('links').innerHTML = `
            <a href="${data.user.links.android}" class="download-button" target="_blank">Android</a>
            <a href="${data.user.links.ios}" class="download-button" target="_blank">iOS</a>
            <a href="${data.user.links.permanent}" class="download-button" target="_blank">Permanent</a>
          `;
        ...
        button.textContent = 'Get User Info';
