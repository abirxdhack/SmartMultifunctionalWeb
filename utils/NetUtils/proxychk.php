...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    ...
    $proxy = isset($_POST['proxy']) ? filter_var($_POST['proxy'], FILTER_SANITIZE_STRING) : '';
    if (!$proxy) {
        echo json_encode(['error' => 'Valid proxy address is required']);
        exit;
    }
    $api_url = "https://a360api-c8fbf2fa3cda.herokuapp.com/net/px?proxy=" . urlencode($proxy);
    $ch = curl_init();
    ...
    if ($data && isset($data['success']) && $data['success'] === true) {
        echo json_encode([
            'status' => 'success',
            'data' => [
                'proxy' => isset($data['data']['proxy']) ? htmlspecialchars($data['data']['proxy']) : '',
                'status' => isset($data['data']['status']) ? htmlspecialchars($data['data']['status']) : '',
                'location' => isset($data['data']['location']) ? htmlspecialchars($data['data']['location']) : '',
                'anonymity' => isset($data['data']['anonymity']) ? htmlspecialchars($data['data']['anonymity']) : ''
            ],
...
<head>
  ...
  <meta name="description" content="Smart Proxy Checker: Check proxy status, anonymity, and location with ease. Developed by Abir Arafat Chawdhury (@ISmartCoder).">
  <meta name="keywords" content="Proxy Checker, Proxy Status, @ISmartCoder">
  <meta name="author" content="Abir Arafat Chawdhury (@ISmartCoder)">
  <title>Smart Proxy Checker</title>
  ...
  <style>
    ...
    .glow {
      ...
      text-shadow: 0 0 12px rgba(0, 0, 0, 0.5), 0 0 24px rgba(0, 0, 0, 0.3);
      letter-spacing: 1px;
    }
    .stylish-text {
      font-family: 'Poppins', sans-serif;
    ...
    @keyframes logoGlow {
      0% { box-shadow: 0 0 10px rgba(0, 0, 0, 0.5), 0 0 20px rgba(0, 0, 0, 0.3); }
      50% { box-shadow: 0 0 25px rgba(0, 0, 0, 0.8), 0 0 35px rgba(0, 0, 0, 0.5); }
      100% { box-shadow: 0 0 10px rgba(0, 0, 0, 0.5), 0 0 20px rgba(0, 0, 0, 0.3); }
    }
    @media (min-width: 640px) {
      .logo {
        max-width: 220px;
    ...
    .footer-container::before {
      ...
      width: 100%;
      height: 100%;
      background: linear-gradient(45deg, rgba(0, 0, 0, 0.1), rgba(16, 185, 129, 0.1));
      opacity: 0.4;
      z-index: 0;
    }
    .footer-content {
    ...
    @keyframes pulseGlow {
      0% { text-shadow: 0 0 15px rgba(0, 0, 0, 0.9); }
      50% { text-shadow: 0 0 20px rgba(0, 0, 0, 1); }
      100% { text-shadow: 0 0 15px rgba(0, 0, 0, 0.9); }
    }
  </style>
</head>
<body>
  ...
  <header class="text-center py-8 sm:py-16">
    <img src="https://i.ibb.co/yBNCxwvM/enhanced.png" alt="Smart Proxy Checker Logo" class="logo">
    <div class="flex items-center justify-center mt-6 sm:mt-8">
      <h1 class="text-4xl sm:text-5xl md:text-6xl font-bold glow stylish-text mr-4">Smart Proxy Checker</h1>
    ...
    <p class="welcome-message mt-3 sm:mt-4 tracking-wide stylish-text text-lg sm:text-xl leading-relaxed max-w-5xl mx-auto">
      The Ultimate Proxy Checker: Verify proxy status, anonymity, and location with ease. Built with passion by @ISmartCoder. Still under active development, daily updated by @TheSmartDev.
    </p>
  ...
  <section id="proxy-checker" class="section-card max-w-5xl mx-auto">
    <h2 class="text-3xl sm:text-4xl font-semibold text-center mb-8 sm:mb-10 glow stylish-text">Proxy Checker</h2>
    <div class="tool-card">
      <div class="tool-card-content">
        <div class="flex justify-center mb-8">
          <i class="fas fa-shield-alt text-6xl text-gray-600 animate-pulse"></i>
        </div>
        <h3 class="text-2xl font-bold stylish-text mb-8 text-center">Check Proxy</h3>
        <div id="form-container" class="mb-8">
          <div class="grid grid-cols-1 max-w-md mx-auto">
            <div>
              <label class="block text-sm font-medium stylish-text mb-3">Proxy Address</label>
              <input type="text" id="proxy-input" class="input-field" placeholder="e.g., 192.168.1.1:8080 or http://192.168.1.1:8080:user:pass" required>
            </div>
          </div>
        </div>
        <div class="text-center">
          <button onclick="checkProxy()" class="button font-bold">Check Proxy</button>
        </div>
        <p id="error" class="error hidden"></p>
        <div id="results" class="hidden">
          <h4 class="text-xl font-bold stylish-text mb-6 text-center">Proxy Details</h4>
          <div class="result-box">
            <p><i class="fas fa-check-circle"></i> <strong>Status:</strong> <span id="status"></span></p>
            <p><i class="fas fa-server"></i> <strong>Proxy:</strong> <span id="proxy"></span></p>
            <p><i class="fas fa-map-marker-alt"></i> <strong>Location:</strong> <span id="location"></span></p>
            <p><i class="fas fa-user-secret"></i> <strong>Anonymity:</strong> <span id="anonymity"></span></p>
          </div>
        </div>
      </div>
    </div>
  </section>
  <footer class="bg-gradient-to-r from-amber-50 to-gray-50 py-10 sm:py-16 mt-auto">
    <div class="footer-container max-w-5xl mx-auto text-center">
      <div class="footer-content">
        ...
        <p class="text-base sm:text-lg font-bold stylish-text">© 2025 Smart Proxy Checker. All rights reserved.</p>
      </div>
    </div>
  </footer>
  <script>
    ...
    const colors = [0x000000, 0x4b5563, 0x10b981];
    for (let i = 0; i < 600; i++) {
    ...
    async function checkProxy() {
      ...
      if (!proxy) {
        errorEl.textContent = 'Please enter a valid proxy address.';
      ...
      try {
        ...
        if (data.error) {
          ...
          document.getElementById('proxy').textContent = data.data.proxy;
          document.getElementById('location').textContent = data.data.location;
          document.getElementById('anonymity').textContent = data.data.anonymity;
        ...
        button.textContent = 'Check Proxy';
