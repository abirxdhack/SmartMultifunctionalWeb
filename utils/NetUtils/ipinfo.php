...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    ...
    $ip = isset($_POST['ip']) ? filter_var($_POST['ip'], FILTER_VALIDATE_IP) : '';
    if (!$ip) {
        echo json_encode(['error' => 'Valid IP address is required']);
        exit;
    }
    $api_url = "https://a360api-c8fbf2fa3cda.herokuapp.com/net/chk?ip=" . urlencode($ip);
    $ch = curl_init();
    ...
    if ($data && isset($data['success']) && $data['success'] === true) {
        echo json_encode([
            'status' => 'success',
            'data' => [
                'ip' => isset($data['data']['ip']) ? htmlspecialchars($data['data']['ip']) : '',
                'asn' => isset($data['data']['asn']) ? htmlspecialchars($data['data']['asn']) : '',
                'isp' => isset($data['data']['isp']) ? htmlspecialchars($data['data']['isp']) : '',
                'country' => isset($data['data']['country']) ? htmlspecialchars($data['data']['country']) : '',
                'city' => isset($data['data']['city']) ? htmlspecialchars($data['data']['city']) : '',
                'timezone' => isset($data['data']['timezone']) ? htmlspecialchars($data['data']['timezone']) : '',
                'fraud_score' => isset($data['data']['fraud_score']) ? (int)$data['data']['fraud_score'] : 0,
                'risk_level' => isset($data['data']['risk_level']) ? htmlspecialchars($data['data']['risk_level']) : ''
            ],
...
<head>
  ...
  <meta name="description" content="Smart IP Checker: Check IP address details with ease. Developed by Abir Arafat Chawdhury (@ISmartCoder).">
  <meta name="keywords" content="IP Checker, IP Lookup, @ISmartCoder">
...
<body>
  ...
  <header class="text-center py-8 sm:py-16">
    <img src="https://i.ibb.co/yBNCxwvM/enhanced.png" alt="Smart IP Checker Logo" class="logo">
    <div class="flex items-center justify-center mt-6 sm:mt-8">
      <h1 class="text-4xl sm:text-5xl md:text-6xl font-bold glow stylish-text mr-4">Smart IP Checker</h1>
    ...
    <p class="welcome-message mt-3 sm:mt-4 tracking-wide stylish-text text-lg sm:text-xl leading-relaxed max-w-5xl mx-auto">
      The Ultimate IP Checker: Get detailed IP address information with ease. Built with passion by @ISmartCoder. Still under active development, daily updated by @TheSmartDev.
    </p>
  ...
  <section id="ip-checker" class="section-card max-w-5xl mx-auto">
    <h2 class="text-3xl sm:text-4xl font-semibold text-center mb-8 sm:mb-10 glow stylish-text">IP Address Checker</h2>
    <div class="tool-card">
      <div class="tool-card-content">
        <div class="flex justify-center mb-8">
          <i class="fas fa-network-wired text-6xl text-gray-600 animate-pulse"></i>
        </div>
        <h3 class="text-2xl font-bold stylish-text mb-8 text-center">Check IP Address</h3>
        <div id="form-container" class="mb-8">
          <div class="grid grid-cols-1 max-w-md mx-auto">
            <div>
              <label class="block text-sm font-medium stylish-text mb-3">IP Address</label>
              <input type="text" id="ip-input" class="input-field" placeholder="e.g., 8.8.8.8" required>
            </div>
          </div>
        </div>
        <div class="text-center">
          <button onclick="checkIP()" class="button font-bold">Check IP</button>
        </div>
        <p id="error" class="error hidden"></p>
        <div id="results" class="hidden">
          <h4 class="text-xl font-bold stylish-text mb-6 text-center">IP Details</h4>
          <div class="result-box">
            ...
            <p><i class="fas fa-server"></i> <strong>ASN:</strong> <span id="asn"></span></p>
            <p><i class="fas fa-network-wired"></i> <strong>ISP:</strong> <span id="isp"></span></p>
            ...
            <p><i class="fas fa-clock"></i> <strong>Timezone:</strong> <span id="timezone"></span></p>
            <p><i class="fas fa-shield-alt"></i> <strong>Fraud Score:</strong> <span id="fraud_score"></span></p>
            <p><i class="fas fa-exclamation-circle"></i> <strong>Risk Level:</strong> <span id="risk_level"></span></p>
          </div>
        </div>
      </div>
    </div>
  ...
  <footer class="bg-gradient-to-r from-indigo-50 to-purple-50 py-10 sm:py-16 mt-auto">
    <div class="footer-container max-w-5xl mx-auto text-center">
      <div class="footer-content">
        ...
        <p class="text-base sm:text-lg font-bold stylish-text">© 2025 Smart IP Checker. All rights reserved.</p>
      </div>
    </div>
  </footer>
  <script>
    ...
    async function checkIP() {
      ...
      try {
        ...
        if (data.error) {
          ...
          document.getElementById('ip').textContent = data.data.ip;
          document.getElementById('asn').textContent = data.data.asn;
          document.getElementById('isp').textContent = data.data.isp;
          ...
          document.getElementById('fraud_score').textContent = data.data.fraud_score;
          document.getElementById('risk_level').textContent = data.data.risk_level;
