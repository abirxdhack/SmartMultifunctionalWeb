...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    ...
    if ($action === 'fetch_gainers') {
        $url = 'https://a360api-c8fbf2fa3cda.herokuapp.com/binance/gainers';
        $ch = curl_init();
        ...
        if ($response === false || $http_code !== 200) {
            echo json_encode(['error' => 'Failed to fetch gainers data: ' . ($curl_error ?: 'HTTP ' . $http_code)]);
            exit;
        }
        ...
        if ($data && isset($data['success']) && $data['success']) {
            ...
            echo json_encode(['error' => 'Invalid API response or no gainers data']);
        }
        exit;
    }
...
<head>
  ...
  <meta name="description" content="CC Xen Binance Gainers: Check top crypto gainers. Developed by Abir Arafat Chawdhury (@ISmartCoder).">
  <meta name="keywords" content="CC Xen Gainers, Binance Gainers, crypto gainers, @ISmartCoder">
  <meta name="author" content="Abir Arafat Chawdhury (@ISmartCoder)">
  <title>Smat Binance Gainers</title>
...
<body>
  ...
  <header class="text-center py-8 sm:py-16">
    ...
    <div class="flex items-center justify-center mt-6 sm:mt-8">
      <h1 class="text-4xl sm:text-5xl md:text-6xl font-bold glow stylish-text mr-4">Smart Binance Gainers</h1>
    ...
    <p class="welcome-message mt-3 sm:mt-4 tracking-wide stylish-text text-lg sm:text-xl leading-relaxed max-w-5xl mx-auto">
      The Ultimate Binance Gainers Checker: Discover top crypto gainers with style. Built with passion by @ISmartCoder. Daily updates by @TheSmartDev.
    </p>
  ...
  <section id="gainers-checker" class="section-card max-w-5xl mx-auto">
    <h2 class="text-3xl sm:text-4xl font-semibold text-center mb-8 sm:mb-10 glow stylish-text">Binance Top Gainers</h2>
    <div class="tool-card">
      <div class="tool-card-content">
        <div class="flex justify-center mb-8">
          <i class="fas fa-trophy text-6xl text-gray-600 animate-pulse"></i>
        </div>
        <h3 class="text-2xl font-bold stylish-text mb-8 text-center">Top Gaining Tokens</h3>
        <div class="text-center mb-6">
          <button onclick="fetchGainers()" class="button font-bold">Refresh Gainers</button>
        </div>
        <p id="error" class="error hidden"></p>
        <div id="results" class="hidden">
          <h4 class="text-xl font-bold stylish-text mb-6 text-center">Top Gainers</h4>
          <div class="result-box">
            <div class="gainers-grid" id="gainers-grid"></div>
          </div>
        </div>
      </div>
    </div>
  ...
  <footer class="bg-gradient-to-r from-indigo-50 to-purple-50 py-10 sm:py-16 mt-auto">
    <div class="footer-container max-w-5xl mx-auto text-center">
      <div class="footer-content">
        ...
        <p class="text-base sm:text-lg font-bold stylish-text">© 2025 Smart Binance Gainers. All rights reserved.</p>
      </div>
    </div>
  </footer>
  <script>
    ...
    async function fetchGainers() {
      ...
      const button = document.querySelector('#gainers-checker .button');
      ...
      formData.append('action', 'fetch_gainers');
      try {
        ...
        if (data.error) {
          ...
          data.data.forEach(gainer => {
            ...
            card.innerHTML = `
              <p><i class="fas fa-chart-line"></i> <strong>Symbol:</strong> ${gainer.symbol} 📈</p>
              <p><i class="fas fa-arrow-up"></i> <strong>Price Change:</strong> ${gainer.priceChange} (${gainer.priceChangePercent}%) 🔼</p>
              <p><i class="fas fa-balance-scale"></i> <strong>Weighted Avg Price:</strong> ${gainer.weightedAvgPrice} 💰</p>
              <p><i class="fas fa-clock"></i> <strong>Prev Close Price:</strong> ${gainer.prevClosePrice} ⏳</p>
              <p><i class="fas fa-dollar-sign"></i> <strong>Last Price:</strong> ${gainer.lastPrice} 💵</p>
              <p><i class="fas fa-shopping-cart"></i> <strong>Last Qty:</strong> ${gainer.lastQty} 🛒</p>
              <p><i class="fas fa-gavel"></i> <strong>Bid Price:</strong> ${gainer.bidPrice} 🔨</p>
              <p><i class="fas fa-box"></i> <strong>Bid Qty:</strong> ${gainer.bidQty} 📦</p>
              <p><i class="fas fa-handshake"></i> <strong>Ask Price:</strong> ${gainer.askPrice} 🤝</p>
              <p><i class="fas fa-cart-plus"></i> <strong>Ask Qty:</strong> ${gainer.askQty} 🛍️</p>
              <p><i class="fas fa-play"></i> <strong>Open Price:</strong> ${gainer.openPrice} 🎬</p>
              <p><i class="fas fa-arrow-up"></i> <strong>High Price:</strong> ${gainer.highPrice} ⬆️</p>
              <p><i class="fas fa-arrow-down"></i> <strong>Low Price:</strong> ${gainer.lowPrice} ⬇️</p>
              <p><i class="fas fa-volume-up"></i> <strong>Volume:</strong> ${gainer.volume} 📢</p>
              <p><i class="fas fa-calculator"></i> <strong>Quote Volume:</strong> ${gainer.quoteVolume} 🧮</p>
              <p><i class="fas fa-clock"></i> <strong>Open Time:</strong> ${new Date(gainer.openTime).toLocaleString()} ⏰</p>
              <p><i class="fas fa-clock"></i> <strong>Close Time:</strong> ${new Date(gainer.closeTime).toLocaleString()} ⏱️</p>
              <p><i class="fas fa-hashtag"></i> <strong>First ID:</strong> ${gainer.firstId} #️⃣</p>
              <p><i class="fas fa-hashtag"></i> <strong>Last ID:</strong> ${gainer.lastId} #️⃣</p>
              <p><i class="fas fa-list"></i> <strong>Count:</strong> ${gainer.count} 📋</p>
            `;
        ...
        button.textContent = 'Refresh Gainers';
        button.disabled = false;
      }
    }
    document.addEventListener('DOMContentLoaded', fetchGainers);
