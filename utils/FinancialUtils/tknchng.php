...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    ...
    $base = isset($_POST['base']) ? strtoupper(filter_var($_POST['base'], FILTER_SANITIZE_STRING)) : '';
    $target = isset($_POST['target']) ? strtoupper(filter_var($_POST['target'], FILTER_SANITIZE_STRING)) : '';
    $amount = isset($_POST['amount']) ? filter_var($_POST['amount'], FILTER_VALIDATE_FLOAT) : 0;
    if (!$base || !$target || $amount <= 0) {
        echo json_encode(['error' => 'Base coin, target coin, and valid amount are required']);
        exit;
    }
    $url = "https://a360api-c8fbf2fa3cda.herokuapp.com/binance/cx?base={$base}&target={$target}&amount={$amount}";
    $ch = curl_init();
    ...
    if ($response === false || $http_code !== 200) {
        echo json_encode(['error' => 'Failed to fetch conversion data: ' . ($curl_error ?: 'HTTP ' . $http_code)]);
        exit;
    }
    ...
    if ($data && isset($data['success']) && $data['success']) {
        ...
        echo json_encode(['error' => 'Invalid conversion data or API error']);
    }
    exit;
}
?>
...
<head>
  ...
  <meta name="description" content="CC Xen Token Converter: Convert crypto tokens with ease. Developed by Abir Arafat Chawdhury (@ISmartCoder).">
  <meta name="keywords" content="CC Xen Converter, crypto conversion, @ISmartCoder">
  <meta name="author" content="Abir Arafat Chawdhury (@ISmartCoder)">
  <title>Smat Token Converter</title>
...
<body>
  ...
  <header class="text-center py-8 sm:py-16">
    ...
    <div class="flex items-center justify-center mt-6 sm:mt-8">
      <h1 class="text-4xl sm:text-5xl md:text-6xl font-bold glow stylish-text mr-4">Smart Token Converter</h1>
    ...
    <p class="welcome-message mt-3 sm:mt-4 tracking-wide stylish-text text-lg sm:text-xl leading-relaxed max-w-5xl mx-auto">
      The Ultimate Token Converter: Convert crypto tokens with ease. Built with passion by @ISmartCoder. Daily updates by @TheSmartDev.
    </p>
  ...
  <section id="converter-checker" class="section-card max-w-5xl mx-auto">
    <h2 class="text-3xl sm:text-4xl font-semibold text-center mb-8 sm:mb-10 glow stylish-text">Token Converter</h2>
    <div class="tool-card">
      <div class="tool-card-content">
        <div class="flex justify-center mb-8">
          <i class="fas fa-exchange-alt text-6xl text-gray-600 animate-pulse"></i>
        </div>
        <h3 class="text-2xl font-bold stylish-text mb-8 text-center">Convert Tokens</h3>
        <div id="form-container" class="mb-8">
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-2xl mx-auto">
            <div>
              <label class="block text-sm font-medium stylish-text mb-3">Base Coin (e.g., TON)</label>
              <input type="text" id="base-input" class="input-field" placeholder="e.g., TON">
            </div>
            <div>
              <label class="block text-sm font-medium stylish-text mb-3">Target Coin (e.g., USDT)</label>
              <input type="text" id="target-input" class="input-field" placeholder="e.g., USDT">
            </div>
            <div>
              <label class="block text-sm font-medium stylish-text mb-3">Amount</label>
              <input type="number" id="amount-input" class="input-field" placeholder="e.g., 10" min="0.01" step="0.01">
            </div>
          </div>
        </div>
        <div class="text-center">
          <button onclick="convertToken()" class="button font-bold">Convert</button>
        </div>
        <p id="error" class="error hidden"></p>
        <div id="results" class="hidden">
          <h4 class="text-xl font-bold stylish-text mb-6 text-center">Conversion Result</h4>
  ...
  <footer class="bg-gradient-to-r from-indigo-50 to-purple-50 py-10 sm:py-16 mt-auto">
    <div class="footer-container max-w-5xl mx-auto text-center">
      <div class="footer-content">
        ...
        <p class="text-base sm:text-lg font-bold stylish-text">© 2025 Smart Binance Token Changer. All rights reserved.</p>
      </div>
    </div>
  </footer>
  <script>
    ...
    async function convertToken() {
      ...
      const button = document.querySelector('#converter-checker .button');
      const base = document.getElementById('base-input').value.trim().toUpperCase();
      const target = document.getElementById('target-input').value.trim().toUpperCase();
      const amount = document.getElementById('amount-input').value;
      if (!base || !target || !amount || amount <= 0) {
        errorEl.textContent = 'Please enter valid base coin, target coin, and amount.';
      ...
      try {
        ...
        if (data.error) {
          ...
          conversionInfo.innerHTML = `
            <p><i class="fas fa-coins"></i> <strong>Base Coin:</strong> ${info.base_coin} 🌐</p>
            <p><i class="fas fa-bullseye"></i> <strong>Target Coin:</strong> ${info.target_coin} 🎯</p>
            <p><i class="fas fa-calculator"></i> <strong>Amount:</strong> ${info.amount} ${info.base_coin} 💱</p>
            <p><i class="fas fa-exchange-alt"></i> <strong>Converted Amount:</strong> ${info.converted_amount} ${info.target_coin} 🔄</p>
            <p><i class="fas fa-money-bill-wave"></i> <strong>Total in USDT:</strong> ${info.total_in_usdt} 💵</p>
            <p><i class="fas fa-chart-line"></i> <strong>Base USDT Price:</strong> ${info.base_usdt_price} 📈</p>
            <p><i class="fas fa-chart-pie"></i> <strong>Target USDT Price:</strong> ${info.target_usdt_price} 📊</p>
          `;
