...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    ...
    $clean_bin = substr($clean_bin, 0, 6);
    $url = "https://data.handyapi.com/bin/{$clean_bin}";
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
    curl_setopt($ch, CURLOPT_HTTPHEADER, ['x-api-key: HAS-0YSb780tq6PMVx7s6jmpQU', 'User-Agent: Mozilla/5.0']);
    ...
    if ($data && isset($data['Status']) && $data['Status'] === 'SUCCESS') {
        $flag_emoji = '';
        $country_code = isset($data['Country']['A2']) ? $data['Country']['A2'] : 'UN';
        $country_name = isset($data['Country']['Name']) ? $data['Country']['Name'] : 'Unknown Country';
        ...
        echo json_encode([
            'status' => $data['Status'],
            'scheme' => isset($data['Scheme']) ? htmlspecialchars($data['Scheme']) : 'Unknown Scheme',
            'type' => isset($data['Type']) ? htmlspecialchars($data['Type']) : 'Unknown Type',
            'issuer' => isset($data['Issuer']) ? htmlspecialchars($data['Issuer']) : 'Unknown Bank',
            'cardTier' => isset($data['CardTier']) ? htmlspecialchars($data['CardTier']) : 'Unknown Tier',
            'country' => "{$country_name} {$flag_emoji}",
            'country_code' => $country_code,
            'country_a3' => isset($data['Country']['A3']) ? htmlspecialchars($data['Country']['A3']) : 'N/A',
            'country_n3' => isset($data['Country']['N3']) ? htmlspecialchars($data['Country']['N3']) : 'N/A',
            'isd' => isset($data['Country']['ISD']) ? htmlspecialchars($data['Country']['ISD']) : 'N/A',
            'continent' => isset($data['Country']['Cont']) ? htmlspecialchars($data['Country']['Cont']) : 'N/A',
...
<head>
  ...
  <meta name="description" content="CC Xen BIN Checker: Verify credit card BIN details with ease. Developed by Abir Arafat Chawdhury (@ISmartCoder).">
  <meta name="keywords" content="CC Xen BIN Checker, BIN validator, credit card validator, @ISmartCoder">
  <meta name="author" content="Abir Arafat Chawdhury (@ISmartCoder)">
  <title>Smat BIN Checker</title>
...
<body>
  ...
  <header class="text-center py-8 sm:py-16">
    ...
    <div class="flex items-center justify-center mt-6 sm:mt-8">
      <h1 class="text-4xl sm:text-5xl md:text-6xl font-bold glow stylish-text mr-4">Smart BIN Checker Tool</h1>
    ...
    <p class="welcome-message mt-3 sm:mt-4 tracking-wide stylish-text text-lg sm:text-xl leading-relaxed max-w-5xl mx-auto">
      The Ultimate BIN Checker: Verify credit card BIN details with ease. Built with passion by @ISmartCoder. Still under active development, daily updated by @TheSmartDev.
    </p>
  ...
  <section id="bin-checker" class="section-card max-w-5xl mx-auto">
    <h2 class="text-3xl sm:text-4xl font-semibold text-center mb-8 sm:mb-10 glow stylish-text">BIN Checker</h2>
    <div class="tool-card">
      <div class="tool-card-content">
        <div class="flex justify-center mb-8">
          <i class="fas fa-search text-6xl text-gray-600 animate-pulse"></i>
        </div>
        <h3 class="text-2xl font-bold stylish-text mb-8 text-center">Verify BIN Details</h3>
        <div id="form-container" class="mb-8">
          <div class="grid grid-cols-1 max-w-md mx-auto">
            <div>
              <label class="block text-sm font-medium stylish-text mb-3">Enter BIN</label>
              <input type="text" id="bin-input" class="input-field" placeholder="e.g., 453201xxxxxx" required>
            </div>
          </div>
        </div>
        <div class="text-center">
          <button onclick="checkBin()" class="button font-bold">Check BIN</button>
        </div>
        <p id="error" class="error hidden"></p>
        <div id="results" class="hidden">
          <h4 class="text-xl font-bold stylish-text mb-6 text-center">BIN Details</h4>
          <div class="result-box">
            <p><i class="fas fa-credit-card"></i> <strong>BIN Status:</strong> <span id="status"></span></p>
            <p><i class="fas fa-globe"></i> <strong>Scheme:</strong> <span id="scheme"></span></p>
            <p><i class="fas fa-id-card"></i> <strong>Type:</strong> <span id="type"></span></p>
            <p><i class="fas fa-university"></i> <strong>Issuer:</strong> <span id="issuer"></span></p>
            <p><i class="fas fa-star"></i> <strong>Card Tier:</strong> <span id="cardTier"></span></p>
            <p><i class="fas fa-flag"></i> <strong>Country:</strong> <span id="country"></span></p>
            <p><i class="fas fa-code"></i> <strong>Country Code (A2):</strong> <span id="country_code"></span></p>
            <p><i class="fas fa-code-branch"></i> <strong>Country Code (A3):</strong> <span id="country_a3"></span></p>
            <p><i class="fas fa-hashtag"></i> <strong>Country Numeric (N3):</strong> <span id="country_n3"></span></p>
            <p><i class="fas fa-phone"></i> <strong>ISD Code:</strong> <span id="isd"></span></p>
            <p><i class="fas fa-map"></i> <strong>Continent:</strong> <span id="continent"></span></p>
            <p><i class="fas fa-check-circle"></i> <strong>Luhn Check:</strong> <span id="luhn"></span></p>
          </div>
        </div>
      </div>
    </div>
  ...
  <footer class="bg-gradient-to-r from-indigo-50 to-purple-50 py-10 sm:py-16 mt-auto">
    <div class="footer-container max-w-5xl mx-auto text-center">
      <div class="footer-content">
        ...
        <p class="text-base sm:text-lg font-bold stylish-text">© 2025 Smart Binance Bin CHK. All rights reserved.</p>
      </div>
    </div>
  </footer>
  <script>
    ...
    async function checkBin() {
      ...
      try {
        ...
        if (data.error) {
          ...
          document.getElementById('country_a3').textContent = data.country_a3;
          document.getElementById('country_n3').textContent = data.country_n3;
          document.getElementById('isd').textContent = data.isd;
          document.getElementById('continent').textContent = data.continent;
          document.getElementById('luhn').textContent = data.luhn;
