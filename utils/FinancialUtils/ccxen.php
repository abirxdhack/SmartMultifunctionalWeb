...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    ...
    $bin = isset($_POST['bin']) ? filter_var($_POST['bin'], FILTER_SANITIZE_STRING) : '';
    $month = isset($_POST['month']) ? filter_var($_POST['month'], FILTER_SANITIZE_STRING) : 'xx';
    $year = isset($_POST['year']) ? filter_var($_POST['year'], FILTER_SANITIZE_STRING) : 'xx';
    $cvv = isset($_POST['cvv']) ? filter_var($_POST['cvv'], FILTER_SANITIZE_STRING) : 'xxx';
    $amount = isset($_POST['amount']) ? filter_var($_POST['amount'], FILTER_VALIDATE_INT) : 10;
    if (!$bin) {
        echo json_encode(['error' => 'BIN is required']);
        exit;
    }
    if ($amount <= 0) {
    ...
    $url = 'https://a360api-c8fbf2fa3cda.herokuapp.com/ccgen?bin=' . urlencode($bin) . '|' . urlencode($month) . '|' . urlencode($year) . '|' . urlencode($cvv) . '&amount=' . urlencode($amount);
    $ch = curl_init();
    ...
    if ($data && isset($data['status']) && $data['status'] === 'success') {
        echo json_encode([
            'binInfo' => isset($data['BIN Info']) ? htmlspecialchars($data['BIN Info']) : 'N/A',
            'bank' => isset($data['Bank']) ? htmlspecialchars($data['Bank']) : 'N/A',
            'country' => isset($data['Country']) ? htmlspecialchars($data['Country']) : 'N/A',
            'cards' => isset($data['cards']) ? array_map('htmlspecialchars', $data['cards']) : []
        ]);
    } else {
        echo json_encode(['error' => 'API error: ' . (isset($data['message']) ? htmlspecialchars($data['message']) : 'Unknown error')]);
    }
    exit;
}
?>
...
<head>
  ...
  <meta name="description" content="CC Xen Generator: Generate multiple credit cards with ease. Developed by Abir Arafat Chawdhury (@ISmartCoder).">
  <meta name="keywords" content="CC Xen Generator, credit card generator, BIN generator, @ISmartCoder">
  <meta name="author" content="Abir Arafat Chawdhury (@ISmartCoder)">
  <title>Smat CC Generator</title>
...
<body>
  ...
  <header class="text-center py-8 sm:py-16">
    ...
    <div class="flex items-center justify-center mt-6 sm:mt-8">
      <h1 class="text-4xl sm:text-5xl md:text-6xl font-bold glow stylish-text mr-4">Smart CC Generator Tool</h1>
    ...
    <p class="welcome-message mt-3 sm:mt-4 tracking-wide stylish-text text-lg sm:text-xl leading-relaxed max-w-5xl mx-auto">
      The Ultimate Credit Card Generator: Generate multiple credit cards with ease. Built with passion by @ISmartCoder. Still under active development, daily updated by @TheSmartDev.
    </p>
  ...
  <section id="cc-generator" class="section-card max-w-5xl mx-auto">
    <h2 class="text-3xl sm:text-4xl font-semibold text-center mb-8 sm:mb-10 glow stylish-text">Credit Card Generator</h2>
    <div class="tool-card">
      <div class="tool-card-content">
        <div class="flex justify-center mb-8">
          <i class="fas fa-credit-card text-6xl text-gray-600 animate-pulse"></i>
        </div>
        <h3 class="text-2xl font-bold stylish-text mb-8 text-center">Generate Credit Cards</h3>
        <div id="form-container" class="mb-8">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              <label class="block text-sm font-medium stylish-text mb-3">BIN</label>
              <input type="text" id="split-bin" class="input-field" placeholder="e.g., 453201xxxxxx" required>
            </div>
            <div>
              <label class="block text-sm font-medium stylish-text mb-3">Month (optional)</label>
              <input type="text" id="split-month" class="input-field" placeholder="e.g., 12 or leave empty for random">
            </div>
            <div>
              <label class="block text-sm font-medium stylish-text mb-3">Year (optional)</label>
              <input type="text" id="split-year" class="input-field" placeholder="e.g., 2027 or leave empty for random">
            </div>
            <div>
              <label class="block text-sm font-medium stylish-text mb-3">CVV (optional)</label>
              <input type="text" id="split-cvv" class="input-field" placeholder="e.g., 123 or leave empty for random">
            </div>
            <div class="sm:col-span-2">
              <label class="block text-sm font-medium stylish-text mb-3">Amount (optional)</label>
              <input type="number" id="split-amount" class="input-field" placeholder="e.g., 10 or leave empty for 10">
            </div>
          </div>
        </div>
        <div class="text-center">
          <button onclick="generateCards()" class="button font-bold">Generate Cards</button>
        </div>
        <p id="error" class="error hidden"></p>
        <div id="results" class="hidden">
          <h4 class="text-xl font-bold stylish-text mb-6 text-center">Results</h4>
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-6">
            <p id="bin-info" class="text-base stylish-text bg-gray-50 p-3 rounded-lg shadow-sm"></p>
            <p id="bank" class="text-base stylish-text bg-gray-50 p-3 rounded-lg shadow-sm"></p>
            <p id="country" class="text-base stylish-text bg-gray-50 p-3 rounded-lg shadow-sm"></p>
          </div>
          <div class="result-box">
            <ul id="cards" class="list-none"></ul>
          </div>
          <div class="text-center mt-6">
            <button onclick="copyCards()" class="copy-button">Copy CCs</button>
          </div>
        </div>
      </div>
    </div>
  ...
  <footer class="bg-gradient-to-r from-indigo-50 to-purple-50 py-10 sm:py-16 mt-auto">
    <div class="footer-container max-w-5xl mx-auto text-center">
      <div class="footer-content">
        ...
        <p class="text-base sm:text-lg font-bold stylish-text">© 2025 CC Xen Generator. All rights reserved.</p>
      </div>
    </div>
  </footer>
  <script>
    ...
    async function generateCards() {
      ...
      const bin = document.getElementById('split-bin').value.trim();
      const month = document.getElementById('split-month').value.trim();
      const year = document.getElementById('split-year').value.trim();
      const cvv = document.getElementById('split-cvv').value.trim();
      const amount = document.getElementById('split-amount').value.trim();
      if (!bin) {
        errorEl.textContent = 'Please enter a valid BIN.';
      ...
      formData.append('input_method', 'split');
      formData.append('bin', bin);
      formData.append('month', month || 'xx');
      formData.append('year', year || 'xx');
      formData.append('cvv', cvv || 'xxx');
      ...
      try {
        ...
        if (data.error) {
          ...
          binInfoEl.textContent = `BIN Info: ${data.binInfo}`;
          bankEl.textContent = `Bank: ${data.bank}`;
      ...
      if (cards) {
        if (navigator.clipboard && navigator.clipboard.writeText) {
          navigator.clipboard.writeText(cards).then(() => {
            alert('Credit cards copied to clipboard!');
          }).catch(err => {
            alert('Failed to copy (Clipboard API): ' + err.message);
            ...
            try {
              ...
              alert('Credit cards copied to clipboard (fallback)!');
            } catch (err) {
              alert('Failed to copy (fallback): ' + err.message);
            }
