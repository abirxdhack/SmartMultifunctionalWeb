...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'search' && isset($_POST['bank'])) {
        ...
        $api_url = "https://a360api-c8fbf2fa3cda.herokuapp.com/bindb/bin?bank=" . urlencode($bank) . "&amount=" . urlencode($amount);
        $ch = curl_init();
        ...
        if ($data && isset($data['results']['status']) && $data['results']['status'] === 'SUCCESS') {
            ...
            echo json_encode([
                'status' => 'ERROR',
                'error' => 'No BIN data found for the specified bank',
...
<head>
  ...
  <meta name="description" content="Smart BIN Bank Search: Find Bank Identification Numbers (BINs) by bank name with a vibrant, futuristic interface. Developed by Abir Arafat Chawdhury (@ISmartCoder).">
  <meta name="keywords" content="BIN Search, Bank Identification Number, Bank Search, @ISmartCoder, Futuristic UI">
  <meta name="author" content="Abir Arafat Chawdhury (@ISmartCoder)">
  <title>Smart BIN Bank Search</title>
...
<body>
  ...
  <header class="text-center py-8 sm:py-16">
    <img src="https://i.ibb.co/yBNCxwvM/enhanced.png" alt="Smart BIN Bank Search Logo" class="logo">
    <div class="flex items-center justify-center mt-6 sm:mt-8">
      <h1 class="text-4xl sm:text-5xl md:text-6xl font-bold glow stylish-text mr-4">Smart BIN Bank Search</h1>
    ...
    <p class="welcome-message mt-3 sm:mt-4 tracking-wide stylish-text text-lg sm:text-xl leading-relaxed max-w-5xl mx-auto">
      The Ultimate BIN Bank Search: Find Bank Identification Numbers by bank name with a vibrant, futuristic interface. Built with passion by @ISmartCoder. Still under active development, daily updated by @TheSmartDev.
    </p>
  ...
  <section id="bin-search" class="section-card max-w-5xl mx-auto">
    <h2 class="text-3xl sm:text-4xl font-semibold text-center mb-8 sm:mb-10 glow stylish-text">BIN Bank Search</h2>
    <div class="tool-card">
      <div class="tool-card-content">
        <div class="flex justify-center mb-8">
          <i class="fas fa-university text-6xl text-green-500 animate-pulse"></i>
        </div>
        <h3 class="text-2xl font-bold stylish-text mb-8 text-center">Search BINs by Bank</h3>
        <div id="form-container" class="mb-8">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-lg mx-auto">
            <div>
              <label class="block text-sm font-medium stylish-text mb-3 text-gray-700">Bank Name</label>
              <input type="text" id="bank-input" class="input-field" placeholder="e.g., PRIME">
            </div>
            <div>
  ...
  <footer class="bg-gradient-to-r from-emerald-50 to-teal-50 py-10 sm:py-16 mt-auto">
    <div class="footer-container max-w-5xl mx-auto text-center">
      <div class="footer-content">
        ...
        <p class="text-base sm:text-lg font-bold stylish-text">© 2025 Smart BIN Bank Search. All rights reserved.</p>
      </div>
    </div>
  </footer>
  <script>
    ...
    async function searchBINs() {
      ...
      if (!bank) {
        errorEl.textContent = 'Please enter a valid bank name.';
      ...
      try {
        ...
        if (data.error) {
          ...
          document.getElementById('filtered-by').textContent = data.bins.length > 0 ? data.bins[0].issuer : 'N/A';
          
          if (data.count < data.requested_amount && data.count > 0) {
            document.getElementById('note-text').textContent = `Requested ${data.requested_amount} results, but only ${data.count} available for ${data.bins[0].issuer}.`;
          ...
          if (data.bins.length === 0) {
            binResultsEl.innerHTML = '<p class="text-center stylish-text text-lg font-medium text-gray-700">No BINs found for the specified bank.</p>';
          } else {
            data.bins.forEach((bin, index) => {
              ...
              binCard.innerHTML = `
                ...
                <p><i class="fas fa-flag"></i> <strong>Country:</strong> <span>${bin.country}</span></p>
              `;
