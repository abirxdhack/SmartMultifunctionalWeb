...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    ...
    if ($action === 'fetch_losers') {
        $url = 'https://a360api-c8fbf2fa3cda.herokuapp.com/binance/losers';
        $ch = curl_init();
        ...
        if ($response === false || $http_code !== 200) {
            echo json_encode(['error' => 'Failed to fetch losers data: ' . ($curl_error ?: 'HTTP ' . $http_code)]);
            exit;
        }
        ...
        if ($data && isset($data['success']) && $data['success']) {
            ...
            echo json_encode(['error' => 'Invalid API response or no losers data']);
        }
        exit;
    }
...
<head>
  ...
  <meta name="description" content="CC Xen Binance Losers: Check top crypto losers. Developed by Abir Arafat Chawdhury (@ISmartCoder).">
  <meta name="keywords" content="CC Xen Losers, Binance Losers, crypto losers, @ISmartCoder">
  <meta name="author" content="Abir Arafat Chawdhury (@ISmartCoder)">
  <title>Smat Binance Losers</title>
  ...
  <style>
    ...
    .loser-card {
      background: rgba(220, 53, 69, 0.1); /* Reddish tint for losers */
      border-radius: 1rem;
      padding: 1.5rem;
      transition: transform 0.3s ease, box-shadow 0.3s ease;
    }
    ...
    .loser-card p {
      ...
      color: #dc3545; /* Red text for losers */
    }
    .loser-card p:hover {
...
<body>
  ...
  <header class="text-center py-8 sm:py-16">
    ...
    <div class="flex items-center justify-center mt-6 sm:mt-8">
      <h1 class="text-4xl sm:text-5xl md:text-6xl font-bold glow stylish-text mr-4">Smart Binance Losers</h1>
    ...
    <p class="welcome-message mt-3 sm:mt-4 tracking-wide stylish-text text-lg sm:text-xl leading-relaxed max-w-5xl mx-auto">
      The Ultimate Binance Losers Checker: Discover top crypto losers with style. Built with passion by @ISmartCoder. Daily updates by @TheSmartDev.
    </p>
  ...
  <section id="losers-checker" class="section-card max-w-5xl mx-auto">
    <h2 class="text-3xl sm:text-4xl font-semibold text-center mb-8 sm:mb-10 glow stylish-text">Binance Top Losers</h2>
    <div class="tool-card">
      <div class="tool-card-content">
        <div class="flex justify-center mb-8">
          <i class="fas fa-arrow-down text-6xl text-gray-600 animate-pulse"></i>
        </div>
        <h3 class="text-2xl font-bold stylish-text mb-8 text-center">Top Losing Tokens</h3>
        <div class="text-center mb-6">
          <button onclick="fetchLosers()" class="button font-bold">Refresh Losers</button>
        </div>
        <p id="error" class="error hidden"></p>
        <div id="results" class="hidden">
          <h4 class="text-xl font-bold stylish-text mb-6 text-center">Top Losers</h4>
          <div class="result-box">
            <div class="losers-grid" id="losers-grid"></div>
          </div>
        </div>
      </div>
    </div>
  ...
  <footer class="bg-gradient-to-r from-indigo-50 to-purple-50 py-10 sm:py-16 mt-auto">
    <div class="footer-container max-w-5xl mx-auto text-center">
      <div class="footer-content">
        ...
        <p class="text-base sm:text-lg font-bold stylish-text">© 2025 Smart Binance Losers. All rights reserved.</p>
      </div>
    </div>
  </footer>
  <script>
    ...
    async function fetchLosers() {
      ...
      const losersGrid = document.getElementById('losers-grid');
      const button = document.querySelector('#losers-checker .button');
      ...
      formData.append('action', 'fetch_losers');
      try {
        ...
        if (data.error) {
          ...
          data.data.forEach(loser => {
            const card = document.createElement('div');
            card.className = 'loser-card';
            card.innerHTML = `
              <p><i class="fas fa-chart-line"></i> <strong>Symbol:</strong> ${loser.symbol} 📉</p>
              <p><i class="fas fa-arrow-down"></i> <strong>Price Change:</strong> ${loser.priceChange} (${loser.priceChangePercent}%) 🔽</p>
              <p><i class="fas fa-balance-scale"></i> <strong>Weighted Avg Price:</strong> ${loser.weightedAvgPrice} 💸</p>
              <p><i class="fas fa-clock"></i> <strong>Prev Close Price:</strong> ${loser.prevClosePrice} ⏳</p>
              <p><i class="fas fa-dollar-sign"></i> <strong>Last Price:</strong> ${loser.lastPrice} 💵</p>
              <p><i class="fas fa-shopping-cart"></i> <strong>Last Qty:</strong> ${loser.lastQty} 🛒</p>
              <p><i class="fas fa-gavel"></i> <strong>Bid Price:</strong> ${loser.bidPrice} 🔨</p>
              <p><i class="fas fa-box"></i> <strong>Bid Qty:</strong> ${loser.bidQty} 📦</p>
              <p><i class="fas fa-handshake"></i> <strong>Ask Price:</strong> ${loser.askPrice} 🤝</p>
              <p><i class="fas fa-cart-plus"></i> <strong>Ask Qty:</strong> ${loser.askQty} 🛍️</p>
              <p><i class="fas fa-play"></i> <strong>Open Price:</strong> ${loser.openPrice} 🎬</p>
              <p><i class="fas fa-arrow-up"></i> <strong>High Price:</strong> ${loser.highPrice} ⬆️</p>
              <p><i class="fas fa-arrow-down"></i> <strong>Low Price:</strong> ${loser.lowPrice} ⬇️</p>
              <p><i class="fas fa-volume-up"></i> <strong>Volume:</strong> ${loser.volume} 📢</p>
              <p><i class="fas fa-calculator"></i> <strong>Quote Volume:</strong> ${loser.quoteVolume} 🧮</p>
              <p><i class="fas fa-clock"></i> <strong>Open Time:</strong> ${new Date(loser.openTime).toLocaleString()} ⏰</p>
              <p><i class="fas fa-clock"></i> <strong>Close Time:</strong> ${new Date(loser.closeTime).toLocaleString()} ⏱️</p>
              <p><i class="fas fa-hashtag"></i> <strong>First ID:</strong> ${loser.firstId} #️⃣</p>
              <p><i class="fas fa-hashtag"></i> <strong>Last ID:</strong> ${loser.lastId} #️⃣</p>
              <p><i class="fas fa-list"></i> <strong>Count:</strong> ${loser.count} 📋</p>
            `;
            losersGrid.appendChild(card);
          });
        ...
        button.textContent = 'Refresh Losers';
        button.disabled = false;
      }
    }
    document.addEventListener('DOMContentLoaded', fetchLosers);
