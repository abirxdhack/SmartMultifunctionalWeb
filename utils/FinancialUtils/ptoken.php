...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    ...
    $token = isset($_POST['token']) ? strtoupper(filter_var($_POST['token'], FILTER_SANITIZE_STRING)) : '';
    if (!$token) {
        echo json_encode(['error' => 'Token is required']);
        exit;
    }
    $url = "https://a360api-c8fbf2fa3cda.herokuapp.com/binance/price?token={$token}";
    $ch = curl_init();
    ...
    if ($response === false || $http_code !== 200) {
        echo json_encode(['error' => 'Failed to fetch token data: ' . ($curl_error ?: 'HTTP ' . $http_code)]);
        exit;
    }
    ...
    if ($data && isset($data['success']) && $data['success']) {
        ...
        echo json_encode(['error' => 'Invalid token or API error']);
    }
    exit;
}
?>
...
<head>
  ...
  <meta name="description" content="CC Xen Token Price Checker: Get premium token price info. Developed by Abir Arafat Chawdhury (@ISmartCoder).">
  <meta name="keywords" content="CC Xen Token Price, crypto price, @ISmartCoder">
  <meta name="author" content="Abir Arafat Chawdhury (@ISmartCoder)">
  <title>Smat Token Price Checker</title>
...
<body>
  ...
  <header class="text-center py-8 sm:py-16">
    ...
    <div class="flex items-center justify-center mt-6 sm:mt-8">
      <h1 class="text-4xl sm:text-5xl md:text-6xl font-bold glow stylish-text mr-4">Smart Token Price Checker</h1>
    ...
    <p class="welcome-message mt-3 sm:mt-4 tracking-wide stylish-text text-lg sm:text-xl leading-relaxed max-w-5xl mx-auto">
      The Ultimate Token Price Checker: Get premium crypto price info with style. Built with passion by @ISmartCoder. Daily updates by @TheSmartDev.
    </p>
  ...
  <section id="token-checker" class="section-card max-w-5xl mx-auto">
    <h2 class="text-3xl sm:text-4xl font-semibold text-center mb-8 sm:mb-10 glow stylish-text">Token Price Checker</h2>
    <div class="tool-card">
      <div class="tool-card-content">
        <div class="flex justify-center mb-8">
          <i class="fas fa-chart-line text-6xl text-gray-600 animate-pulse"></i>
        </div>
        <h3 class="text-2xl font-bold stylish-text mb-8 text-center">Check Token Price</h3>
        <div id="form-container" class="mb-8">
          <div class="grid grid-cols-1 max-w-md mx-auto">
            <div>
              <label class="block text-sm font-medium stylish-text mb-3">Enter Token (e.g., BTC, ETH)</label>
              <input type="text" id="token-input" class="input-field" placeholder="e.g., BTC">
            </div>
          </div>
        </div>
        <div class="text-center">
          <button onclick="fetchTokenPrice()" class="button font-bold">Check Price</button>
        </div>
        <p id="error" class="error hidden"></p>
        <div id="results" class="hidden">
          <h4 class="text-xl font-bold stylish-text mb-6 text-center">Token Price Info</h4>
  ...
  <footer class="bg-gradient-to-r from-indigo-50 to-purple-50 py-10 sm:py-16 mt-auto">
    <div class="footer-container max-w-5xl mx-auto text-center">
      <div class="footer-content">
        ...
        <p class="text-base sm:text-lg font-bold stylish-text">© 2025 Smart Binance Token Price. All rights reserved.</p>
      </div>
    </div>
  </footer>
  <script>
    ...
    async function fetchTokenPrice() {
      ...
      const button = document.querySelector('#token-checker .button');
      ...
      try {
        ...
        if (data.error) {
          ...
          tokenInfo.innerHTML = `
            <p><i class="fas fa-chart-line"></i> <strong>Symbol:</strong> ${info.symbol} 📈</p>
            <p><i class="fas fa-arrow-up"></i> <strong>Price Change:</strong> ${info.priceChange} (${info.priceChangePercent}%) 🔼</p>
            <p><i class="fas fa-balance-scale"></i> <strong>Weighted Avg Price:</strong> ${info.weightedAvgPrice} 💰</p>
            <p><i class="fas fa-clock"></i> <strong>Prev Close Price:</strong> ${info.prevClosePrice} ⏳</p>
            <p><i class="fas fa-dollar-sign"></i> <strong>Last Price:</strong> ${info.lastPrice} 💵</p>
            <p><i class="fas fa-shopping-cart"></i> <strong>Last Qty:</strong> ${info.lastQty} 🛒</p>
            <p><i class="fas fa-gavel"></i> <strong>Bid Price:</strong> ${info.bidPrice} 🔨</p>
            <p><i class="fas fa-box"></i> <strong>Bid Qty:</strong> ${info.bidQty} 📦</p>
            <p><i class="fas fa-handshake"></i> <strong>Ask Price:</strong> ${info.askPrice} 🤝</p>
            <p><i class="fas fa-cart-plus"></i> <strong>Ask Qty:</strong> ${info.askQty} 🛍️</p>
            <p><i class="fas fa-play"></i> <strong>Open Price:</strong> ${info.openPrice} 🎬</p>
            <p><i class="fas fa-arrow-up"></i> <strong>High Price:</strong> ${info.highPrice} ⬆️</p>
            <p><i class="fas fa-arrow-down"></i> <strong>Low Price:</strong> ${info.lowPrice} ⬇️</p>
            <p><i class="fas fa-volume-up"></i> <strong>Volume:</strong> ${info.volume} 📢</p>
            <p><i class="fas fa-calculator"></i> <strong>Quote Volume:</strong> ${info.quoteVolume} 🧮</p>
            <p><i class="fas fa-clock"></i> <strong>Open Time:</strong> ${new Date(info.openTime).toLocaleString()} ⏰</p>
            <p><i class="fas fa-clock"></i> <strong>Close Time:</strong> ${new Date(info.closeTime).toLocaleString()} ⏱️</p>
            <p><i class="fas fa-hashtag"></i> <strong>First ID:</strong> ${info.firstId} #️⃣</p>
            <p><i class="fas fa-hashtag"></i> <strong>Last ID:</strong> ${info.lastId} #️⃣</p>
            <p><i class="fas fa-list"></i> <strong>Count:</strong> ${info.count} 📋</p>
          `;
