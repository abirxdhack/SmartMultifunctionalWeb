...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    ...
    if (!$url) {
        echo json_encode(['error' => 'Instagram URL is required']);
        exit;
    }
    $api_url = "https://a360api-c8fbf2fa3cda.herokuapp.com/insta/dl?url=" . urlencode($url);
    $ch = curl_init();
    ...
    if ($data && isset($data['status']) && $data['status'] === 'success') {
        echo json_encode([
            'status' => $data['status'],
            'media_count' => isset($data['media_count']) ? (int)$data['media_count'] : 0,
            'results' => array_map(function($item) {
                return [
                    'thumbnail' => isset($item['thumbnail']) ? htmlspecialchars($item['thumbnail']) : '',
                    'downloadLink' => isset($item['downloadLink']) ? htmlspecialchars($item['downloadLink']) : '',
                    'type' => isset($item['type']) ? htmlspecialchars($item['type']) : 'unknown',
                    'filename' => isset($item['filename']) ? htmlspecialchars($item['filename']) : 'unknown'
                ];
            }, $data['results'] ?? []),
...
<head>
  ...
  <meta name="description" content="Smart Instagram Downloader: Download Instagram posts and reels with ease. Developed by Abir Arafat Chawdhury (@ISmartCoder).">
  <meta name="keywords" content="Instagram Downloader, Instagram Post Downloader, Instagram Reel Downloader, @ISmartCoder">
  <meta name="author" content="Abir Arafat Chawdhury (@ISmartCoder)">
  <title>Smart Instagram Downloader</title>
...
<body>
  ...
  <header class="text-center py-8 sm:py-16">
    <img src="https://i.ibb.co/yBNCxwvM/enhanced.png" alt="Smart Instagram Downloader Logo" class="logo">
    <div class="flex items-center justify-center mt-6 sm:mt-8">
      <h1 class="text-4xl sm:text-5xl md:text-6xl font-bold glow stylish-text mr-4">Smart Instagram Downloader</h1>
    ...
    <p class="welcome-message mt-3 sm:mt-4 tracking-wide stylish-text text-lg sm:text-xl leading-relaxed max-w-5xl mx-auto">
      The Ultimate Instagram Downloader: Download Instagram posts and reels with ease. Built with passion by @ISmartCoder. Still under active development, daily updated by @TheSmartDev.
    </p>
  ...
  <section id="insta-downloader" class="section-card max-w-5xl mx-auto">
    <h2 class="text-3xl sm:text-4xl font-semibold text-center mb-8 sm:mb-10 glow stylish-text">Instagram Post & Reel Downloader</h2>
    <div class="tool-card">
      <div class="tool-card-content">
        <div class="flex justify-center mb-8">
          <i class="fab fa-instagram text-6xl text-gray-600 animate-pulse"></i>
        </div>
        <h3 class="text-2xl font-bold stylish-text mb-8 text-center">Download Instagram Media</h3>
        <div id="form-container" class="mb-8">
          <div class="grid grid-cols-1 max-w-md mx-auto">
            <div>
              <label class="block text-sm font-medium stylish-text mb-3">Instagram URL</label>
              <input type="url" id="url-input" class="input-field" placeholder="e.g., https://www.instagram.com/p/XXXXX/" required>
            </div>
          </div>
        </div>
        <div class="text-center">
          <button onclick="downloadMedia()" class="button font-bold">Download Media</button>
        </div>
        <p id="error" class="error hidden"></p>
        <div id="results" class="hidden">
          <h4 class="text-xl font-bold stylish-text mb-6 text-center">Media Details</h4>
          <p><i class="fas fa-check-circle"></i> <strong>Status:</strong> <span id="status"></span></p>
          <p><i class="fas fa-photo-video"></i> <strong>Media Count:</strong> <span id="media_count"></span></p>
  ...
  <footer class="bg-gradient-to-r from-indigo-50 to-purple-50 py-10 sm:py-16 mt-auto">
    <div class="footer-container max-w-5xl mx-auto text-center">
      <div class="footer-content">
        ...
        <p class="text-base sm:text-lg font-bold stylish-text">© 2025 Smart Instagram Downloader. All rights reserved.</p>
      </div>
    </div>
  </footer>
  <script>
    ...
    async function downloadMedia() {
      ...
      try {
        ...
        if (data.error) {
          ...
          document.getElementById('media_count').textContent = data.media_count;
          data.results.forEach((item, index) => {
            const mediaCard = document.createElement('div');
            mediaCard.className = 'media-card';
            mediaCard.innerHTML = `
              <p><i class="fas fa-${item.type === 'image' ? 'image' : 'video'}"></i> <strong>Media ${index + 1} (${item.type}):</strong> ${item.filename}</p>
              ${item.type === 'image' ? `<img src="${item.thumbnail}" alt="Media Thumbnail">` : `<video src="${item.thumbnail}" autoplay muted loop playsinline></video>`}
              <a href="${item.downloadLink}" class="download-button" target="_blank">Download ${item.type}</a>
            `;
        ...
        button.textContent = 'Download Media';
