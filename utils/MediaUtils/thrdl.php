...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    ...
    if (!$url) {
        echo json_encode(['error' => 'Threads video URL is required']);
        exit;
    }
    $api_url = "https://a360api-c8fbf2fa3cda.herokuapp.com/thrd/thd?url=" . urlencode($url);
    $ch = curl_init();
    ...
    if ($data && isset($data['results']) && !empty($data['results']['video_urls'])) {
        echo json_encode([
            'status' => 'success',
            'links' => array_map(function($item, $index) {
                return [
                    'url' => htmlspecialchars($item['download_url']),
                    'filename' => 'threads_video_' . ($index + 1) . '.mp4'
                ];
...
<head>
  ...
  <meta name="description" content="Smart Threads Video Downloader: Download Threads videos with ease. Developed by Abir Arafat Chawdhury (@abirxdhack).">
  <meta name="keywords" content="Threads Video Downloader, Threads Downloader, @abirxdhack">
  <meta name="author" content="Abir Arafat Chawdhury (@abirxdhack)">
  <title>Smart Threads Video Downloader</title>
...
<body>
  ...
  <header class="text-center py-8 sm:py-16">
    <img src="https://i.ibb.co/yBNCxwvM/enhanced.png" alt="Smart Threads Video Downloader Logo" class="logo">
    <div class="flex items-center justify-center mt-6 sm:mt-8">
      <h1 class="text-4xl sm:text-5xl md:text-6xl font-bold glow stylish-text mr-4">Smart Threads Video Downloader</h1>
    ...
    <p class="welcome-message mt-3 sm:mt-4 tracking-wide stylish-text text-lg sm:text-xl leading-relaxed max-w-5xl mx-auto">
      The Ultimate Threads Video Downloader: Download Threads videos with ease. Built with passion by @abirxdhack. Still under active development, daily updated by @TheSmartDev.
    </p>
  ...
  <section id="threads-downloader" class="section-card max-w-5xl mx-auto">
    <h2 class="text-3xl sm:text-4xl font-semibold text-center mb-8 sm:mb-10 glow stylish-text">Threads Video Downloader</h2>
    <div class="tool-card">
      <div class="tool-card-content">
        <div class="flex justify-center mb-8">
          <i class="fab fa-threads text-6xl text-gray-600 animate-pulse"></i>
        </div>
        <h3 class="text-2xl font-bold stylish-text mb-8 text-center">Download Threads Video</h3>
        <div id="form-container" class="mb-8">
          <div class="grid grid-cols-1 max-w-md mx-auto">
            <div>
              <label class="block text-sm font-medium stylish-text mb-3">Threads Video URL</label>
              <input type="url" id="url-input" class="input-field" placeholder="e.g., https://www.threads.com/@username/post/XXXXX" required>
            </div>
          </div>
        </div>
  ...
  <footer class="bg-gradient-to-r from-amber-50 to-gray-50 py-10 sm:py-16 mt-auto">
    <div class="footer-container max-w-5xl mx-auto text-center">
      <div class="footer-content">
        ...
        <p class="text-base sm:text-lg font-bold stylish-text">© 2025 Smart Threads Video Downloader. All rights reserved.</p>
      </div>
    </div>
  </footer>
  <script>
    ...
    async function downloadVideo() {
      ...
      if (!url) {
        errorEl.textContent = 'Please enter a valid Threads video URL.';
