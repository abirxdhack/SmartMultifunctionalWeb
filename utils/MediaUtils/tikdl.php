...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    ...
    if (!$url) {
        echo json_encode(['error' => 'TikTok video URL is required']);
        exit;
    }
    $api_url = "https://a360api-c8fbf2fa3cda.herokuapp.com/tik/dl?url=" . urlencode($url);
    $ch = curl_init();
...
<head>
  ...
  <meta name="description" content="Smart TikTok Video Downloader: Download TikTok videos with ease. Developed by Abir Arafat Chawdhury (@ISmartCoder).">
  <meta name="keywords" content="TikTok Video Downloader, TikTok Downloader, @ISmartCoder">
  <meta name="author" content="Abir Arafat Chawdhury (@ISmartCoder)">
  <title>Smart TikTok Video Downloader</title>
...
<body>
  ...
  <header class="text-center py-8 sm:py-16">
    <img src="https://i.ibb.co/yBNCxwvM/enhanced.png" alt="Smart TikTok Video Downloader Logo" class="logo">
    <div class="flex items-center justify-center mt-6 sm:mt-8">
      <h1 class="text-4xl sm:text-5xl md:text-6xl font-bold glow stylish-text mr-4">Smart TikTok Video Downloader</h1>
    ...
    <p class="welcome-message mt-3 sm:mt-4 tracking-wide stylish-text text-lg sm:text-xl leading-relaxed max-w-5xl mx-auto">
      The Ultimate TikTok Video Downloader: Download TikTok videos with ease. Built with passion by @ISmartCoder. Still under active development, daily updated by @TheSmartDev.
    </p>
  ...
  <section id="tiktok-downloader" class="section-card max-w-5xl mx-auto">
    <h2 class="text-3xl sm:text-4xl font-semibold text-center mb-8 sm:mb-10 glow stylish-text">TikTok Video Downloader</h2>
    <div class="tool-card">
      <div class="tool-card-content">
        <div class="flex justify-center mb-8">
          <i class="fab fa-tiktok text-6xl text-gray-600 animate-pulse"></i>
        </div>
        <h3 class="text-2xl font-bold stylish-text mb-8 text-center">Download TikTok Video</h3>
        <div id="form-container" class="mb-8">
          <div class="grid grid-cols-1 max-w-md mx-auto">
            <div>
              <label class="block text-sm font-medium stylish-text mb-3">TikTok Video URL</label>
              <input type="url" id="url-input" class="input-field" placeholder="e.g., https://www.tiktok.com/@username/video/XXXXX" required>
            </div>
          </div>
        </div>
  ...
  <footer class="bg-gradient-to-r from-indigo-50 to-purple-50 py-10 sm:py-16 mt-auto">
    <div class="footer-container max-w-5xl mx-auto text-center">
      <div class="footer-content">
        ...
        <p class="text-base sm:text-lg font-bold stylish-text">© 2025 Smart TikTok Video Downloader. All rights reserved.</p>
      </div>
    </div>
  </footer>
  <script>
    ...
    async function downloadVideo() {
      ...
      if (!url) {
        errorEl.textContent = 'Please enter a valid TikTok video URL.';
      ...
      try {
        ...
        if (data.error) {
          ...
          const firstVideoLink = data.links.find(link => link.filename.includes('.mp4') && !link.filename.includes('_hd.mp4')) || data.links[0];
          mediaCard.innerHTML = `
            <video src="${firstVideoLink.url}" autoplay muted loop playsinline></video>
            <div>
              ${data.links.map((link, index) => `
                <a href="${link.url}" class="download-button" target="_blank">Download ${link.filename.includes('.mp4') ? (link.filename.includes('_hd.mp4') ? 'HD Video' : 'Video') : 'Audio'}</a>
              `).join('')}
            </div>
          `;
